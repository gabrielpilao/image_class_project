...
valid_transforms = transforms.Compose([transforms.Resize(225), transforms.CenterCrop(224),
                                       transforms.ToTensor(),
                                       transforms.Normalize([0.485, 0.456, 0.406],
                                                             [0.229, 0.224, 0.225])])
...
classifier = nn.Sequential(nn.Linear(25088,4096),
                      nn.Dropout(p=0.3), nn.ReLU(),
                      nn.Linear(4096,4096), nn.Dropout(p=0.3), nn.ReLU(),
                      nn.Linear(4096,1000), nn.Dropout(p=0.3), nn.ReLU(), nn.Linear(1000,102),
                      nn.LogSoftmax(dim=1))
...
for e in range(epochs):
    for images, labels in trainloader:
        ...
        optmizer.zero_grad()
        train_ouput = model.forward(images)
        train_loss = criterion(train_ouput, labels)
...
print('ps: {}'.format(ps),
        'top_p: {}'.format(top_p),
        'top_class: {}'.format(top_class),
        'equal: {}'.format(equal),
        'accuracy: {}'.format(accuracy), sep= '/n')
